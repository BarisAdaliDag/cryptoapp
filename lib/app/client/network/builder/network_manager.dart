...
class NetworkManager extends NetworkManagerProtocol {
  ...
  @override
  NetworkManagerProtocol setRequestMethod({required RequestMethodEnum requestMethodEnum}) {
  ...
  ResultResponse<Response, BaseNetworkErrorType> executeResponse() async {
    ...
    if (chekcNetwork) {
      try {
        ...
        return Result.failure(BaseNetworkErrorType.type(error: typeError.toString()));
      }
    } else {
      if (isLog == true) {
        //   log('${const Result.failure(BaseNetworkErrorType.connectivity(error: 'No Internet Connection'))}');
      }
      return const Result.failure(BaseNetworkErrorType.connectivity(error: 'No Internet Connection'));
    }
  }

  @override
  NetworkManagerProtocol setQueryParameters({required Map<String, dynamic> queryParameters}) {
  ...
  @override
  ResultDecode<K, BaseNetworkErrorType> execute<T extends BaseResponseModel, K>(T responseModel) async {
    try {
      final response = await _dio.fetch(
        RequestOptions(
          path: _path ?? '',
      ...
      final decodeResponse = NetworkDecoding.decode<T, K>(response: response, responseModel: responseModel);
